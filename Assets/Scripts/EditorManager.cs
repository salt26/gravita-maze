...
#region Remove object
    ...
    public void EditQuit(int phase)
    {
        ...
        if (dirtyBit)
        {
            // Í≤ΩÍ≥† Î©îÏãúÏß? ?ùÑ?ö∞Í∏?
            if (phase == (int)EditPhase.Initialize)
            {
                // Îß? Î≥?Í≤? ?õÑ 1?éò?ù¥Ï¶àÏùò ?ÇòÍ∞?Í∏? Î≤ÑÌäº?ùÑ ?àÑÎ•¥Îäî Í≤ΩÏö∞
            ...
            {
                // Îß? Î≥?Í≤? ?õÑ Í≤?Ï¶ùÏ?? ?ôÑÎ£åÌñàÏß?Îß? ????û•?êòÏß? ?ïä??? ?ÉÅ?Éú?óê?Ñú 3?éò?ù¥Ï¶àÏùò ?ÇòÍ∞?Í∏? Î≤ÑÌäº?ùÑ ?àÑÎ•¥Îäî Í≤ΩÏö∞
                messageUI.Initialize("<b>Unsaved Changes</b>\n\nThe map has not been saved yet.\nDo you want to quit anyway?", () => GameManager.gm.LoadMain(), null);
            }
            else
            {
                // Îß? Î≥?Í≤? ?õÑ Í≤?Ï¶ùÏùÑ ?ïòÏß? ?ïäÍ≥? 3?éò?ù¥Ï¶àÏùò ?ÇòÍ∞?Í∏? Î≤ÑÌäº?ùÑ ?àÑÎ•¥Îäî Í≤ΩÏö∞
    ...
    public void EditOpenPhase()
    {
        ...
        // UI ÎßåÎì§Í∏?
        // Maps ?è¥?çî?ùò Î™®Îì† ÎßµÏùÑ Î∂àÎü¨????Ñú Î™©Î°ù?óê ?ùÑ?õåÏ£ºÍ∏∞
    ...
    public void EditSave()
    {
        ...
        try
        {
            ...
            {
                // Í∞ôÏ?? ?ù¥Î¶ÑÏùò ?åå?ùº?ù¥ ?ûà?äî?ç∞ Í∑∏Îûò?èÑ ????û•?ï† Í≤ÉÏù∏Ïß? Î©îÏãúÏß?Î°? Î¨ºÏñ¥Î≥¥Í∏∞
...
#pragma warning disable CS0162 // ?†ëÍ∑ºÌï† ?àò ?óÜ?äî ÏΩîÎìúÍ∞? ?ûà?äµ?ãà?ã§.
...
#pragma warning restore CS0162 // ?†ëÍ∑ºÌï† ?àò ?óÜ?äî ÏΩîÎìúÍ∞? ?ûà?äµ?ãà?ã§.
...
#pragma warning restore CS0162 // ?†ëÍ∑ºÌï† ?àò ?óÜ?äî ÏΩîÎìúÍ∞? ?ûà?äµ?ãà?ã§.
    ...
    private class EditActionInfo
    {
        ...
        /// Type: MapName
        /// </summary>
        /// <param name="oldMapName">?óÜ?úºÎ©? ""</param>
        /// <param name="newMapName">?óÜ?úºÎ©? ""</param>
        ...
        /// Type: SizeX, SizeY
        /// </summary>
        /// <param name="isX">sizeX Î≥?Í≤? ?ãú true, sizeY Î≥?Í≤? ?ãú false</param>
        ...
        /// Type: Wall
        /// </summary>
        /// <param name="oldWallInfo">?óÜ?úºÎ©? null</param>
        /// <param name="newWallInfo">?óÜ?úºÎ©? null</param>
        ...
        /// Type: Object
        /// </summary>
        /// <param name="oldObjectInfo">?óÜ?úºÎ©? null</param>
        /// <param name="newObjectInfo">?óÜ?úºÎ©? null</param>
        ...
        /// </summary>
        /// <param name="oldRemovedWalls">ÔøΩÔøΩÔøΩÔøΩÔøΩÔøΩ null</param>
        /// <param name="oldRemovedObjects">ÔøΩÔøΩÔøΩÔøΩÔøΩÔøΩ null</param>
